/*
 * Copyright (c) 2018. Ivan Widyan - All Rights Reserved
 * Unauthorized copying of this file, via any medium is strictly prohibited
 * Proprietary and confidential
 * Email: ivanwidyan@yahoo.com
 */

package com.testing.blibli.constants;

public class BlibliWebElementConstants {
    // Home
    public final static String HOME_AD = "close-modal";
    public final static String HOME_SEARCHPARAM = "searchParam";
    public final static String HOME_SEARCH_BUTTON = "gdn-search-button";

    // Search
    public final static String SEARCH_SORT_ELEMENT = "sortSelectElement";
    public final static String SEARCH_PRODUCT_TITLE = "product-title";
    public final static String SEARCH_PRODUCT_PRICE = "product-price";
    public final static String SEARCH_LOADING = "ajax-loading";

    // Sort
    public final static String TEXT_PRODUK_REKOMENDASI = "Produk rekomendasi";
    public final static String TEXT_RELEVANSI = "Relevansi";
    public final static String TEXT_PRODUK_TERBARU = "Produk terbaru";
    public final static String TEXT_PRODUK_TERPOPULER = "Produk terpopuler";
    public final static String TEXT_POTONGAN_DISKON = "Potongan diskon";
    public final static String TEXT_HARGA_TERMURAH = "Harga termurah";
    public final static String TEXT_HARGA_TERMAHAL = "Harga termahal";
    public final static String TEXT_RATING_TERTINGGI = "Rating Tertinggi";

    public final static String SORT_PRODUK_REKOMENDASI = "11";
    public final static String SORT_RELEVANSI = "0";
    public final static String SORT_PRODUK_TERBARU = "1";
    public final static String SORT_PRODUK_TERPOPULER = "7";
    public final static String SORT_POTONGAN_DISKON = "6";
    public final static String SORT_HARGA_TERMURAH = "3";
    public final static String SORT_HARGA_TERMAHAL = "4";
    public final static String SORT_RATING_TERTINGGI = "9";

    // Filter - Brand
    public final static String FILTER_BRAND = "search-brand";

    // Filter - Price
    public final static String FILTER_PRICE_MIN = "min-price-filter-input";
    public final static String FILTER_PRICE_MAX = "max-price-filter-input";

    // Filter - Location
    public final static String FILTER_LOCATION = "location-filter";

    // Summary
    public final static String SUMMARY_SIZE = "attributes--size";
    public final static String SUMMARY_QUANTITY = "quantity__increase";
    public final static String SUMMARY_ADD_TO_CART = "add-to-cart__button";
    public final static String SUMMARY_BUY_NOW = "buy-now__button";

    // Delivery
    public final static String DELIVERY_NEXT_STEP = "gdn-next-step";

    // Payment
    public final static String PAYMENT_TRANSFER = "gdn-payment-category-Transfer";
    public final static String PAYMENT_INDOMARET = "gdn-payment-category-Indomaret";
    public final static String PAYMENT_CHECKOUT = "gdn-submit-checkout";

    // Payment - Transfer
    public final static String TRANSFER_BANK_BCA = "BankBCA";
    public final static String TRANSFER_BANK_BRI = "BankBRI";
    public final static String TRANSFER_BANK_DANAMON = "BankDanamon";
    public final static String TRANSFER_BANK_MANDIRI = "BankMandiri";
    public final static String TRANSFER_ATM_BERSAMA = "ATM Bersama0.5575202052338333_CODE";
    public final static String TRANSFER_BANK_BNI = "BankBNI";

    // ThankYou
    public final static String THANKYOU_LAYER = "layer";

}
